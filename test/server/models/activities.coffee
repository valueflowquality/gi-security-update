  ...
  describe 'Activities', ->
    ...
    describe 'Schema', ->
      ...
      it 'status: ObjectId', (done) ->
        expect(mocks.mongoose.model.calledWith('Activity', sinon.match.hasOwn 'status'
      ...
      it 'parent: {key: ObjectId, resourceType: String}', (done) ->
        expect(mocks.mongoose.model.calledWith('Activity', sinon.match.hasOwn 'parent'
        , {key: 'ObjectId', resourceType: 'String'})).to.be.true
        done()
      
      it 'user: {type: ObjectId, ref: User}', (done) ->
        expect(mocks.mongoose.model.calledWith('Activity', sinon.match.hasOwn 'user'
      ...
      it 'job:  {type: ObjectId, ref: Job}', (done) ->
        expect(mocks.mongoose.model.calledWith('Activity'
        , sinon.match.hasOwn 'job', {type: 'ObjectId', ref: 'Job'})).to.be.true
        done()
