...
module.exports = () ->
  describe 'Models', ->
    ...
    beforeEach (done) ->
      ...
      models = proxyquire(dir + '/models', stubs)(mongooseMock, crudModelFactoryMock)

      done()

    it 'exports activities model', (done) ->
      assert.ok stubs['./activities'].calledOnce
      assert stubs['./activities'].calledWithExactly(mongooseMock, crudModelFactoryMock), 'activities not initalized'
    ...
    it 'exports categories model', (done) ->
      ...
      assert stubs['./categories'].calledWithExactly(mongooseMock, crudModelFactoryMock, filesStub), 'categories not initalized'
      assert.property models, 'categories', 'models does not export categories'
      expect(models.categories.name).to.equal 'Category'
      done()

    it 'exports systems model', (done) ->
      assert.ok stubs['./systems'].calledOnce
      assert stubs['./systems'].calledWithExactly(mongooseMock, crudModelFactoryMock), 'systems not initalized'
      assert.property models, 'systems', 'models does not export systems'
      expect(models.systems.name).to.equal 'System'
      done()

    it 'exports environments model', (done) ->
      assert.ok stubs['./environments'].calledOnce
      assert stubs['./environments'].calledWithExactly(mongooseMock, crudModelFactoryMock), 'environments not initalized'
      assert.property models, 'environments', 'models does not export environments'
      expect(models.environments.name).to.equal 'Environment'
      done()

    it 'exports files model', (done) ->
      assert.ok stubs['./files'].calledOnce
      assert stubs['./files'].calledWithExactly(mongooseMock, crudModelFactoryMock), 'files not initalized'
    ...
    it 'exports users model', (done) ->
      ...
      assert stubs['./users'].calledWithExactly(mongooseMock, crudModelFactoryMock), 'users not initalized'
    ...
    it 'exports roles model', (done) ->
      ...
      assert stubs['./roles'].calledWithExactly(mongooseMock, crudModelFactoryMock), 'roles not initalized'
    ...
    it 'exports settings model', (done) ->
      ...
      assert stubs['./settings'].calledWithExactly(mongooseMock, crudModelFactoryMock, environmentsStub), 'settings not initalized'
